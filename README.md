...
위장표는 **파일 › 새 문서** 에서 두 가지 중에 고릅니다. 고른 값은 저장됩니다.
창이 좁아 메뉴줄이 숨은 경우에는 툴바 왼쪽의 **메뉴** 버튼으로 들어갑니다.
...
- 붙여넣을 때 누른 **줄바꿈은 그대로 지켜집니다.** 노래 가사나 시처럼
  행 나눔이 정해진 글도 원래 모양대로 연습할 수 있습니다.
- 여러 줄을 붙여넣으면 **셀이 줄 수만큼 늘어나 한 줄씩 펼쳐집니다.**
  거기 보이는 줄이 곧 시트1에서 칠 줄이라, 줄바꿈이 제대로 들어갔는지
  시트에서 바로 확인할 수 있습니다. (줄바꿈 표시 `↵` 는 수식 입력줄에서 봅니다)
- **인터넷에서 긁어온 글도 줄바꿈을 살려 넣습니다.**
  사이트에 따라 클립보드의 일반 텍스트에 줄바꿈이 하나도 안 담겨 오는데
  (복사를 막으려고 클립보드를 다시 쓰는 사이트가 그렇습니다),
  그럴 때는 같이 실려 온 HTML 을 뜯어 줄을 되살립니다.
  메모장에 옮겼다 다시 복사할 필요가 없습니다.
  사이트가 끼워 넣은 `무단 전재 금지` 같은 문구는 딸려 들어가지 않습니다.
- **줄이 뭉개져 들어와도 셀 안에서 나눌 수 있습니다.**
  셀을 더블클릭하면 여러 줄 편집칸이 열립니다. 나눌 자리에서 `Alt+Enter`
  (또는 `Ctrl+Enter`) 를 누르면 그 자리에서 줄이 나뉘고, 칸과 행이 같이 자랍니다.
  `Enter` 로 확정, `Esc` 로 취소합니다. **메모장을 열 일이 없습니다.**
  이미 여러 줄인 칸을 열면 `↵` 가 아니라 진짜 줄로 펼쳐져 나옵니다.
- 셀을 더블클릭해 편집칸에 바로 붙여넣어도 줄바꿈이 살아남습니다.
- **한 줄이 100자를 넘을 때만** 문장 단위로 더 나눕니다.
  (따옴표 안에서는 끊지 않고, 그래도 긴 문장만 어절 단위로 나눕니다)
- 원래 줄바꿈이 없는 글은 한 줄로 들어갑니다. 셀에서도 한 줄로 보이므로
  줄이 안 들어갔다는 것을 그 자리에서 알 수 있습니다.
...
- **공유** 버튼 → 타임어택 기록을 타수가 높은 순으로 **10명까지** 보여 줍니다.
  프로필의 아이디와 사진이 기록에 함께 남습니다.
  기록은 이 브라우저에만 쌓이므로, `기록 복사` 로 글로 옮겨 남에게 보낼 수 있습니다.
- **업그레이드** 버튼 → 업데이트 이력을 큼직한 것만 모아 보여 줍니다.
...
- 툴바(빠른 설정)는 창이 좁아지면 **세로 점 세 개(⋮)** 안으로 접힙니다.
  단, **글 선택 도구와 메뉴 버튼은 접히지 않고 항상 툴바에 남습니다.**
- `파일 · 수정 · 보기 …` 메뉴 줄은 창이 좁아지면 사라지고,
  툴바의 **메뉴** 버튼에서 같은 메뉴로 들어갈 수 있습니다.
